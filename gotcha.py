...
def get_page():
    s = requests.Session()
    r = s.get(REQUESTS["target"], headers=REQUESTS["headers"])
    return r.text


def get_msg(html, msg=None):
    if msg == None: msg = EmailMessage()
    ...
    lis = soup.select("ul[id=feed-main-list] li")
    for li in lis:
        # tag 标签
        ...
        if img_src:
            ...
            img_src = PAGE["gift"]
        img = requests.get(img_src, headers=REQUESTS["headers"]).content
        ...
        msg.get_payload()[-1].add_related(img, "image", "png", cid=cid)
...
def send_email(msg):
    ...
    server.login(EMAIL["sender"], EMAIL["passwd"])
    server.send_message(msg)
    server.quit()


if __name__ == "__main__":
    send_email(get_msg(get_page()))
    print("Done!")
